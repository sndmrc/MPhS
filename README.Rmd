...
```{r step0, results="hide", warning=FALSE}
library(MPhS)
library(tidyr)
library(dplyr)
data("RPKMdata")
...
```{r step1, results="hide", warning=FALSE}
...
```{r step2, results="hide", warning=FALSE}
dts <- t(RPKMdata[, -1])
...
```{r step4, results="hide", warning=FALSE}
...
```{r plot, results="hide", warning=FALSE}
p <- plot(MPhS_out)
print(p)
```
